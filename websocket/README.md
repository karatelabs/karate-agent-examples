...
1. **The engine** — `karate-async-2.1.2.RC4.jar` from the
...
```bash
javac -cp ../karate-async-2.1.2.RC4.jar -d server-classes server/*.java

java -cp "../karate-async-2.1.2.RC4.jar:server-classes" EchoServer 8090
java -cp "../karate-async-2.1.2.RC4.jar:server-classes" StompBroker 8091
```

Run the checks:

```bash
java -cp "../karate-async-2.1.2.RC4.jar:server-classes" io.karatelabs.Main run checks
```
