...
docker run -d --name karate-agent -p 4444:4444 -v "$PWD":/work \
  public.ecr.aws/karatelabs/karate-agent:2.1.2.RC2
```

**Or the plain JAR** (Java 21+) — download `karate-agent-2.1.2.RC2.jar` from
<https://github.com/karatelabs/karate-addons/releases> (release tag `agent-2.1.2.RC2`), then from
this folder:

```bash
java -jar karate-agent-2.1.2.RC2.jar serve
```
