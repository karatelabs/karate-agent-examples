...
| [`checkout`](checkout/) | **consumer-side contract testing** — a checkout service that depends on a payments API another team owns: the mock the team builds against, an independently implemented real provider, and the **paired run** that proves the mock can stand in — including one deliberate divergence the suite's assertions cannot see, which the pair reports anyway. Consumer-driven contracts with a readable functional suite instead of pact files, and git instead of a broker. **[The live Contract page](https://karatelabs.github.io/karate-agent-examples/checkout/ext/contract/pages/contract.html)** is this kit's CI output — the measured divergence set, browsable. |
