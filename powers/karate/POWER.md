...
The second one is the point of this power. Anything can run a test; the reason to reach for Karate is the evidence it leaves behind. 20 workflows ship with the server — `Skill.list()` on the live server is the authoritative roster.
